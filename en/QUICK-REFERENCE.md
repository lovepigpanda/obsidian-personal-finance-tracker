...
| Category | English Name | Emoji | Keywords |
|----------|--------------|-------|----------|
- Transfer | Transfer | 🔁 | transfer, send to, pay to, transfer in, transfer out, move to, allocate, position transfer, top up, withdraw, credit card payment, pay card, top up position, from, to |

> **Note**: Transfer ≠ expense/income. When a transfer intent is detected, the paired-file workflow must be followed (see the "Transfer Files" section below).
...
**Pairing ID rule**: `T-{YYYY-MM-DD}-{random-string}` — both files **MUST** share the same ID.
...
### Transfer (paired files, from/to swapped, matching pair ID)
```yaml
---
type: transfer
date: YYYY-MM-DD
...
currency: CNY
category: Transfer
from_account: AccountName        # source account
to_account: AccountName          # destination account
transfer_pair_id: T-YYYY-MM-DD-xxx  # ⚠️ must be identical across both files
note: "User's raw description"
...
2. Identify type: expense ("spent","bought","paid","花","买") → expense
                   ...
                   transfer ("transfer","send","allocate","credit card payment","from X to Y") → transfer
...
9. Write to corresponding directory (expense→expenses/, income→incomes/, transfer→transfers/out/ + transfers/in/)
10. Return completion info to user (including pair ID for transfers)
```
...
- Transfer rules | `Categories/transfer-categories.md` |
- Expense template | `Templates/expense-template.md` |
- Income template | `Templates/income-template.md` |
- Transfer template | `Templates/transfer-template.md` |
